...
#include "SimplnxReview/Filters/Algorithms/EMsoftSO3Sampler.hpp"
...
Parameters EMsoftSO3SamplerFilter::parameters() const
{
  ...
  // params.linkParameters(k_SampleModeSelector_Key, k_CellAttributeMatrixName_Key, static_cast<ChoicesParameter::ValueType>(0));
  params.linkParameters(k_SampleModeSelector_Key, k_EnsembleAttributeMatrixPath_Key, static_cast<ChoicesParameter::ValueType>(0));
  ...
  params.linkParameters(k_SampleModeSelector_Key, k_Mode1Misorientation_Key, static_cast<ChoicesParameter::ValueType>(2));
  params.linkParameters(k_SampleModeSelector_Key, k_Mode1EulerAngle_Key, static_cast<ChoicesParameter::ValueType>(2));
                                                               ...
                                                               const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  if(psampleModeSelectorValue == orientation_sampling::k_FZModeIndex)
  {
  ...
  if(psampleModeSelectorValue == orientation_sampling::k_ConstantModeIndex || psampleModeSelectorValue == orientation_sampling::k_LessThanModeIndex)
  {
    numTuples = std::pow((2 * pNumspValue) + 1, 3);
    ...
    if(auto result = euler.isValid(); result.result != 1)
    {
      return MakePreflightErrorResult(-70003, result.msg);
    }
  }
  ...
  {
    auto createArrayAction = std::make_unique<CreateArrayAction>(DataType::float32, std::vector<usize>{numTuples}, std::vector<usize>{3}, pEulerAnglesArrayPath);
                                             ...
                                             const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  inputValues.EulerAnglesArrayPath = filterArgs.value<DataPath>(k_EulerAnglesArrayPath_Key);
