...
#include "SimplnxReview/Filters/Algorithms/ReadZeissTxmFile.hpp"
...
Parameters ReadZeissTxmFileFilter::parameters() const
{
  ...
  params.insert(std::make_unique<UInt32Parameter>(k_SubVolumeStartSlice_Key, "Slice Start", "The starting slice to import", 0));
  params.insert(std::make_unique<UInt32Parameter>(k_SubVolumeEndSlice_Key, "Slice End (inclusive)", "The ending slice to import (inclusive)", 0));
                                                               ...
                                                               const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  {
    return {MakeErrorResult<OutputActions>(-33530, fmt::format("The start slice '{}' is greater than the slice end '{}'", pSliceStart, pSliceEnd))};
  }
  if(pUseSubVolume && pSliceStart >= metadata.Dimensions[2])
  {
    return {MakeErrorResult<OutputActions>(-33531, fmt::format("The start slice '{}' is greater than the total number of slices", pSliceStart, metadata.Dimensions[2]))};
  }
  if(pUseSubVolume && pSliceEnd >= metadata.Dimensions[2])
  {
    return {MakeErrorResult<OutputActions>(-33532, fmt::format("The end slice '{}' is greater than the total number of slices", pSliceEnd, metadata.Dimensions[2]))};
  }
  if(pUseSubVolume && pSliceStart < 1)
  {
    return {MakeErrorResult<OutputActions>(-33533, fmt::format("The start slice '{}' must be 1 or greater", pSliceStart))};
  }
  ...
  if(pUseSubVolume)
  {
    ...
    finalOrigin[2] = metadata.Spacing[2] * finalDimensions[2];
  }
  ...
  preflightUpdatedValues.push_back({"Input Geometry Info", nx::core::GeometryHelpers::Description::GenerateGeometryInfo(finalDimensions, metadata.Spacing, metadata.Origin, metadata.Units)});
