...
{
  ContainerType rho{-rod[0] * rod[3], -rod[1] * rod[3], -rod[2] * rod[3]};
  ...
  return {0.0, 0.0, 0.0, 0.0}; // Return something that is not-valid
}
} // namespace
...
Result<> EMsoftSO3Sampler::operator()()
{
  ...
  if(m_InputValues->sampleModeSelector != orientation_sampling::k_FZModeIndex)
  {
    ...
    if(m_InputValues->sampleModeSelector == orientation_sampling::k_ConstantModeIndex)
    {
  ...
  auto& m_EulerAngles = m_DataStructure.getDataRefAs<Float32Array>(m_InputValues->EulerAnglesArrayPath);
  ...
  if(m_InputValues->sampleModeSelector == orientation_sampling::k_FZModeIndex)
  {
    auto ops = ebsdlib::LaueOps::GetAllOrientationOps();
    ...
    materialNames.setValue(1, ops[m_InputValues->CrystalStructureIndex]->getSymmetryName());
  }
